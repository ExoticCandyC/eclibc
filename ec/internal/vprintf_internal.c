...
#define __ec_printf_int(_type, _base, _upperCase)                              \
    {                                                                          \
        ...
        if((__ec_printf_Number_set & 1) == 0)                                  \
            rightNumber = 0;                                                   \
        NumBuffer = ec_itoa_##_type (EC_SCRATCH(_type), NumBufferEnd,          \
                                                     _base, _upperCase);       \
        NumBuffer = ec_pad_num_string(rightNumber, NumBuffer, NumBufferEnd);   \
        ec_fpad_string(__stream, leftNumber, ' ', NumBuffer, NumBufferEnd);    \
    }
...
#define __ec_printf_double()                                                   \
    {                                                                          \
        ...
        if((__ec_printf_Number_set & 1) == 0)                                  \
            rightNumber = 6;                                                   \
        NumBuffer = ec_ftoa(EC_SCRATCH(double), NumBufferEnd, rightNumber);    \
        ec_fpad_string(__stream, leftNumber, ' ', NumBuffer, NumBufferEnd);    \
    }
...
#define __ec_printf_string()                                                   \
    {                                                                          \
        ...
        if((__ec_printf_Number_set & 2) == 0)                                  \
            leftNumber = 0;                                                    \
        if((__ec_printf_Number_set & 1) == 0 || (size_t)rightNumber > __temp)  \
            rightNumber = (int)__temp;                                         \
        ec_fpad_string(__stream, leftNumber, ' ', EC_SCRATCH(char *),          \
                                        (EC_SCRATCH(char *) + rightNumber));   \
    }
                            ...
                            const char *__restrict __format, va_list __arg)
{
    ...
    NumBufferEnd = NumBuffer_Storage + 80;
    *NumBufferEnd = '\0';
